...
###########
   ...
   # VARIABLES
	...
	var cost {1..T} >= 0;							# Cost in Euro
	var revenue {1..T} >= 0;						# revenue in Euro
	var profit {1..T};								# profit in Euro
	var fullcycle {1..T} >= 0;						# Number of equvalent full-cycles
...
#######################
	...
	subject to total_cost {t in 1..T}:				cost[t] = Intraday_Continuous_15_minutes_Average_Price[t] * bat_in[t] * step * (1 - sto_loss_charge) / 1000;
	subject to total_revenue {t in 1..T}:			revenue[t] = Intraday_Continuous_15_minutes_Average_Price[t] * bat_out[t] * step * (1 - sto_loss_discharge) / 1000;

	subject to profit_new_value {t in 1..T}:			profit[t]  = (revenue[t] - cost[t]);
	...
	subject to th_discharge_limit1 {t in 1..T} :   		sto_min_discharge * sto_discharge[t] <= bat_out[t];
	subject to th_discharge_limit2 {t in 1..T} : 	    bat_out[t] <= sto_discharge[t] * sto_max_discharge;
	subject to th_charge_limit1 {t in 1..T} : 	    	sto_min_charge * sto_charge[t] <= bat_in[t];
	subject to th_charge_limit2 {t in 1..T} : 	    	bat_in[t] <= sto_charge[t] * sto_max_charge; 

# calcultion charge and discharge values for STORAGE 
	
	subject to th1 {t in 1..T}: 				sto_charge[t] + sto_discharge[t]  <= 1;			               # do not charge and discharge at the same time
	subject to th2 : 					        sto_soc[0] = sto_soc_start;				                       # define storage SOC at t=0
	subject to th3 {t in 1..T} : 			    bat_change[t] = bat_out[t] - bat_in[t];   	# separating energy flow in input and output
	

# to equal HeiPhoSS program
	subject to th4 {t in 1..T} :				sto_soc[t] = sto_soc[(t-1)] * (1 - (sto_loss * step)) - (sto_out[t] - sto_in[t]) / sto_cap_max;   
												            
	subject to th5 {t in 1..T} : 				sto_cap[t] = sto_soc[t] * sto_cap_max; 			# calculate storage capacity


	subject to bat_change_1 {t in 1..T} :		sto_in[t]  = bat_in[t] * step * (1 - sto_loss_charge);	# bat_in equals storage_in minus losses
	subject to bat_change_2 {t in 1..T} :		sto_out[t] = bat_out[t] * step / (1 - sto_loss_discharge);		# bat_out equals storage_out plus losses
end;