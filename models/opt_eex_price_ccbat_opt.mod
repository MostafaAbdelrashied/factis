...
###########
  ...
  var sto_cap_c_o {0..T};							# Storage capacity over time in kWh.
  var bat_change_c_o {0..T};						# Power buffer change (output positive). Difference between storage input and output.
  var Intraday_Auction_15_minute_call_o {0..T};							# Power buffer change (output positive). Difference between storage input and output.


  var sto_cap_cc {0..T};							# Storage capacity over time in kWh.
  var bat_change_cc {0..T};						# Power buffer change (output positive). Difference between storage input and output.
...
#########################
  ...
  var sto_cap_o_o {0..T};							    # Storage capacity over time in kWh.
  var bat_change_o_o {0..T};							# Power buffer change (output positive). Difference between storage input and output.
  var Day_Ahead_Auction_o_o {0..T};							# Power buffer change (output positive). Difference between storage input and output.
...
# INTRADAY CORRECTURE CONSTRAINTS
	...
	subject to combined_storage9 {t in 1..T} :   		bat_change_c_o[t] +  bat_change_o_o[t] + bat_change_cc[t] = bat_change[t];
	subject to combined_storage10 {t in 1..T} : 		sto_cap_c_o[t] + sto_cap_o_o[t] + sto_cap_cc[t] = sto_cap[t]; 			# calculate storage capacity
